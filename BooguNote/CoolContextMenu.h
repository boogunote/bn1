...
protected:
    ...
    void DrawItem (LPDRAWITEMSTRUCT lpDrawItemStruct)
	{
		...
		{   
			...
			int nColorIndex = 0;
			bool bFind = false;
			for (; nColorIndex<sizeof(PreDefineColor)/sizeof(COLOR_PAIR); nColorIndex++)
			{
				if (lpDrawItemStruct->itemID == PreDefineColor[nColorIndex].uMsgId)
				{
					bFind = true;
					break;
				}
			...
			if (bFind)
			{
				...
				dc.SetTextColor(PreDefineColor[nColorIndex].TextColor);
				dc.SetBkColor(PreDefineColor[nColorIndex].BkgColor);
			}
			else
			{
            ...
            if (g_config.imageList != NULL && pmd->iImage >= 0)
            {
                bool bOver = !bDisabled && bSelected;

                if (bDisabled || (bSelected && !bChecked))
                {
                    HICON hIcon = ImageList_ExtractIcon(NULL, g_config.imageList, pmd->iImage);
                    CBrush brush;
                ...
                if (!bDisabled)
                {
                    ::ImageList_Draw(g_config.imageList, pmd->iImage, pDC->m_hDC,
