...
void XMLElement :: Copy(UINT uFormat)
	{
	...
	GlobalUnlock(hG);
	SetClipboardData(uFormat, hG);
	CloseClipboard();
#endif
	}

XMLElement* XML :: Paste(char* txt, UINT uFormat)
	{
	if (txt)
		{
		XML* xm = new XML();
...
int XML :: Save(const TCHAR* file,XML_SAVE_MODE SaveMode,XML_TARGET_MODE TargetMode,class XMLTransform* eclass,class XMLTransformData* edata)
	{
	if (TargetMode == 1)
		{
		return 1;
...
#endif
		x[C] = ch;
		C += ch->GetAllChildren(x + C + 1,deep == 0xFFFFFFFF ? deep : (deep - 1));
		C++;
		}
