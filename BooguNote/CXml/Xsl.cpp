...
namespace JWXml
{
	...
	//-------------------------------------------------------------------------
	// Function Name    :OpenText
	// Parameter(s)     :LPCTSTR lpszXslText	XSL file path
	// Return           :BOOL
	// Create			:2008-1-16 16:09 Gao. Xiang
	// Memo             :Open the xsl file
	//-------------------------------------------------------------------------
	BOOL CXsl::OpenText(LPCTSTR lpszXslText)
	{
		Close();
		...
		try
		{
			...
			VARIANT_BOOL vbSuccessful = VARIANT_TRUE;
			vbSuccessful = m_pStyleSheet->loadXML( _bstr_t(lpszXslText) );
			if( vbSuccessful == VARIANT_FALSE ) { ASSERT(FALSE); return FALSE; }
		...
		{
			TRACE( _T("CXsl::OpenText failed:%s\n"), e.ErrorMessage());
			ASSERT( FALSE );
			return FALSE;
		}
	}
